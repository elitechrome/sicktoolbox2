...
namespace SickToolbox {
  ...
  protected:
    ...
    int _readBytes( uint8_t * const dest_buffer, const int num_bytes_to_read, const unsigned int timeout_value = 0 ) const throw ( SickTimeoutException, SickIOException );       
   ...
   */
  template< class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
  int SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_readBytes( uint8_t * const dest_buffer, const int num_bytes_to_read, const unsigned int timeout_value ) const
...
if (num_bytes_to_read == -1)
{
	while(1)
	{
		...
		/* Wait for the OS to tell us that data is waiting! */
                num_active_files = select(getdtablesize(),&file_desc_set,0,0, &timeout_val);
		...
		if (FD_ISSET(_sick_fd,&file_desc_set)) {
		  ...
		  /* Read a single byte from the stream! */
                  num_bytes_read = read(_sick_fd,&dest_buffer[total_num_bytes_read],20);
		  ...
		  if (num_bytes_read > 0) { //Update the number of bytes read so far
		    ...
		    throw SickIOException("SickBufferMonitor::_readBytes: read() failed!");
                  ...
                  for (i; i < total_num_bytes_read; i++)
                  {
                      if (dest_buffer[i] == 0x03)
                      {
                          return i;
                      }
                  }
